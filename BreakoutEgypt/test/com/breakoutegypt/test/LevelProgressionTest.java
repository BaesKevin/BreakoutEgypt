...
public class LevelProgressionTest {
    ...
    public void textGetReachedLevelDefaultNotOpen() {
        ...
        progressions.addNewProgression(GameType.TEST, new LevelProgression());
        LevelProgression prog = progressions.getProgression(GameType.TEST);
    ...
    public void gameStartsAtLevelOneByDefault(){
        ...
        int startingLevel = 1;
        int id = new GameManager().createGame(numberOfPlayerse, startingLevel, GameType.TEST, GameDifficulty.MEDIUM);
    ...
    public void gameCrashesWhenIllegalLevelIsSet(){
        ...
        Game game = new GameManager().getGame(id);
        player = new Player(new User(playername));
        player.getProgressions().addNewProgression(GameType.TEST, new LevelProgression());
        
        game.addConnectingPlayer(player);
        game.addConnectionForPlayer("kevin", new DummyConnection());
        
        game.setCurrentLevel(DEFAULT_OPEN_TEST_LEVELS + 1, player.getProgressions().getProgression(GameType.TEST));
    ...
    public void gameIncrementsLevelReachedOnLevelCompletion(){
        ...
        LevelProgression playerProgression = player.getProgressions().getProgression(GameType.TEST);
        ...
        int levelReached = player.getProgressions().getProgression(GameType.TEST).getHighestLevelReached();
