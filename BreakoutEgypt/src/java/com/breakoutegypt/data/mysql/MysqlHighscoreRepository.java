...
public class MysqlHighscoreRepository implements HighscoreRepository {
    ...
    private final String INSERT_SCORE = "insert into level_scores(levelid, levelNumber,userid,difficultyid,time,points) values(?, ?, ?, ?, ?, ?)";
    ...
    public List<Score> getScoresByLevel(int levelID, String diff) {
        try (
            ...
            while (rs.next()) {
                ...
                s = new Score(scoreid, levelid, levelNumber, p, time, diff, points);
                scores.add(s);
            }
            return scores;
    ...
    public void addScore(Score s) {
        try (
            ...
            prep.setInt(1, s.getLevelId());
            prep.setInt(2, s.getLevelNumber());
            prep.setInt(3, s.getUserId());
            prep.setInt(4, Repositories.getDifficultyRepository().findByName(s.getDifficulty()).getId());
            prep.setLong(5, s.getTimeScore());
            prep.setInt(6, s.getBrickScore());
